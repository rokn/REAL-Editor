...
namespace XnaEmbeded
{
	...
	abstract public class GraphicsDeviceControl : Control
	{
		...
		public RenderTarget2D DefaultRenderTarget => _renderTarget;
		...
		protected GraphicsDeviceControl() : base()
		{
		}
		...
		public Viewport Viewport => new Viewport(0, 0, ClientSize.Width, ClientSize.Height);
		...
		string HandleDeviceReset()
		{
			...
			if(deviceNeedsReset)
			{
				try
				{
					_graphicsDeviceService.ResetDevice(_renderTarget.Width,
		...
		private readonly object _afterDrawEventLock = new object();
		private EventHandler _afterDrawEvent;
		public event EventHandler AfterDraw
		{
			add { lock (_afterDrawEventLock) {
				if (_afterDrawEvent != null) _afterDrawEvent += value;
			} }
			remove { lock (_afterDrawEventLock) {
				if (_afterDrawEvent != null) _afterDrawEvent -= value;
			} }
		}
		protected virtual void OnAfterDraw(EventArgs e)
		{
			...
			lock (_afterDrawEventLock)
			{
				handler = _afterDrawEvent;
				if(handler == null)
					return;
			}
			handler(this, e);
